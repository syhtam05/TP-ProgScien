...
public class Helder extends SysLin {
    ...
    public Helder(Matrice M, Vecteur b) throws IrregularSysLinException {
	super(M, b);
    }
	...
	double[][] dataA = { { 1, 1, -2 }, { 4, -2, 1 }, { 3, -1, 3 } };
	double[] dataB = { 3, 5, 8 };
	...
	Matrice A_pour_calcul = new Matrice(new double[][] { { 1, 1, -2 }, { 4, -2, 1 }, { 3, -1, 3 } });
	...
	double[][] dataA_2 = { { 1, 1, -2 }, { 4, -2, 1 }, { 3, -1, 3 } };
	double[] dataB_2 = { 3, 5, 8 };

	Matrice A_originale_2 = new Matrice(dataA_2);
	Vecteur b_original_2 = new Vecteur(new double[] { 3, 5, 8 });;

	try {
	    System.out.println("--- Résolution de A^2x = b ---");

	    // Étape 1 : Résoudre Ay = b
	    Matrice A_pour_calcul2 = new Matrice(new double[][] { { 1, 1, -2 }, { 4, -2, 1 }, { 3, -1, 3 } });
	    Helder solveur1 = new Helder(A_pour_calcul2, new Vecteur(dataB_2));
	    Vecteur y = solveur1.resolution();

	    // Étape 2 : Résoudre Ax = y
	    A_pour_calcul2 = new Matrice(new double[][] { { 1, 1, -2 }, { 4, -2, 1 }, { 3, -1, 3 } });
	    Helder solveur2 = new Helder(A_pour_calcul2, y);
	    Vecteur x = solveur2.resolution();

	    System.out.println("Solution x finale :");
	    System.out.println(x);

	    // Vérification : A * (A * x) doit être égal à b
	    Matrice AA = Matrice.produit(A_originale_2, A_originale_2);
	    Vecteur.testSolution(AA, x, b_original_2);
